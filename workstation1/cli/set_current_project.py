...
def set_current_project(project_registry: list[dict[str, Any]], step_registry_entry: dict[str, Any]) -> None:
    ...
    if not os.path.isfile(config_file):
        log.warn(f"Cannot set current project using step {step_registry_entry['step_id']} because it does not have a config file") 
        return
    ...
    log.info(f"Setting current project id to {step_project_id}")
    ...
    if not project_found:
        log.warn(f"Project id {step_registry_entry['project_id']} not found in project registry. Could not set current project root.")    
